...
def log_perf(
    *,
    ...
    """Log one readable timing line for a processed document.

    Example: ``perf laporan.pdf: total=67.0s pdf=0.8s excel=0.5s extract=45.3s
    compare=12.1s typo=8.4s (9 fakta, 5 sumber, 3 hal)``.
    """
    durations = timer.durations()
    stages = " ".join(f"{s}={durations[s]:.1f}s" for s in STAGE_ORDER if s in durations)
    logger.info(
        "perf %s: total=%.1fs %s (%d fakta, %d sumber, %d hal)",
        pdf_filename, timer.total(), stages, n_facts, n_excel_sources, n_pages,
    )
