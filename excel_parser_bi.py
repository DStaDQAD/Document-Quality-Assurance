...
Table structure (consistent across monthly updates — only new rightmost columns are added):
  ...
  Row N+2+: Data rows
             col 0 = row index (numeric), col 1 = indent hint, col 2 = KETERANGAN label
...
def _read_xls(data: bytes, sheet_name: str):
    ...
    row_data: List[Tuple[str, Dict[int, float]]] = []
    ...
    return title, unit, col_period_map, row_data
...
def _read_xlsx(data: bytes, sheet_name: str):
    ...
    title = str(all_rows[0][0] or "").strip()
    ...
    for label, vals in row_data:
        ...
        for col, (year, month) in col_period_map.items():
            if col in vals:
                ...
                # First occurrence wins for duplicate labels (sub-items with same name).
