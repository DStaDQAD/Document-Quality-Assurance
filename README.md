# Fact-Checker (Text-to-SQL + LLM-as-a-Judge)

Verifies natural-language claims about statistical data against a SQLite database, using an
LLM to translate each claim into SQL (rather than a fixed query template) and a second LLM call
to judge the claim against whatever the query actually returns.
...
| `/api/verify-document-pdf` | POST | Same as above, but the document is an uploaded digital PDF (no OCR - scanned PDFs aren't supported). |
